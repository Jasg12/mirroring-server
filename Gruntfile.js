module.exports = function(grunt) {
    ...
    var home = grunt.option("mirroring-server_home") || __dirname ;
    grunt.initConfig({
        gitinfo: {
            commands: {
        ...
        shell: {
            build: {
                ...
                //    'cd ..',
                    'mvn clean package',
                    'pwd',
                    'cp -r target/*.jar deploy/mirroring-server.jar'
                ].join('&&')
            },
            build_docker_image: {
                command: [
                    ...
                    'cd ' + home,
                    'docker build --tag="amatsarskisjsu/mirroring-server:<%= gitinfo.my.custom.command %>" .'
                ].join('&&'),
                options: {
                    execOptions: {
                        maxBuffer: Infinity
            ...
            push_docker_image: {
                command: [
                    ...
                    'docker login -u amatsarskisjsu -p 123ZXCzxc',
                    'docker push amatsarskisjsu/mirroring-server:<%= gitinfo.my.custom.command %>'
                ].join('&&')
            }
        },
